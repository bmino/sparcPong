...
describe('team lookup by team id', function() {
    ...
    it('requires authentication', function() {
        return requester
            .get(`/api/team/fetch/${TestUtils.getPlayers()[0]._id}`)
            .then((res) => {
    ...
    it('completes successfully', function() {
        ...
        return requester
            .get(`/api/team/fetch/${TEAM_TO_FETCH._id}`)
            .set('Authorization', `JWT ${token1}`)
            .then((res) => {
                expect(res).to.have.status(200);
                expect(res.body).to.contain.keys('message');
                expect(res.body.message).to.be.an('object');
                expect(res.body.message.leader, 'leader').to.be.an('object');
                expect(res.body.message.partner, 'partner').to.be.an('object');
                expect(res.body.message).to.have.all.keys(Object.keys(TEAM_TO_FETCH));
            });
    });

    it('provides an empty list for an invalid teamId', function() {
        return requester
            .get('/api/team/fetch/F00000000000000000000000')
            .set('Authorization', `JWT ${token1}`)
            .then((res) => {
                expect(res).to.have.status(200);
                expect(res.body).to.contain.keys('message');
